...
class Nanny(ServerNode):
    ...
    def __init__(  # type: ignore[no-untyped-def]
        ...
        **worker_kwargs,
        ...
        logging.info("worker_port:{}".format(worker_port))
        logging.info("worker_class:{}".format(worker_class))
        if loop is not None:
            warnings.warn(
    ...
    async def start_unsafe(self):
        ...
        logging.info("instantiate start...")
        response = await self.instantiate()
        logging.info("instantiate done!!!")
    ...
    async def instantiate(self) -> Status:
        ...
        if self.process is None:
            ...
            logging.info("self.worker={}, {}".format(self.Worker, self.pre_spawn_env))
...
class WorkerProcess:
    ...
    async def start(self) -> Status:
        ...
        logging.info("wait_until_connected...")
        try:
            msg = await self._wait_until_connected(uid)
            logging.info("wait_until_connected done...")
        ...
        logging.info("wait_until_connected pass...")
        if not msg:
            return self.status
    ...
    def _run(
        ...
        try:
            ...
            def watch_stop_q():
                ...
                worker cleanly.
                """
                logging.info("in thread start watch_stop_q...")
                try:
            ...
            thread.daemon = True
            # ERROR_ERROR dask.config.set({'distributed.worker.multiprocessing-method': 'fork'})
            logging.info("start threading...")
            thread.start()
            logging.info("done threading...")

            logging.info("Done define run()")
            async def run():
                """
                Try to start worker and inform parent of outcome.
                """
                try:
                    logging.info("await worker...")
                    await worker
                except Exception as e:
